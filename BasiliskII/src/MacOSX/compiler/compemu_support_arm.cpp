 ...
 * per-thread write-protection call while code is emitted. */
static void uae_vm_jit_write_protect(bool enable)
{
	pthread_jit_write_protect_np(enable ? 1 : 0);
}

static size_t uae_vm_page_size(void)
{
...
static void jit_diag_dump_native_block(uae_u32 guest_pc, int level,
    const uae_u8 *start, const uae_u8 *end)
{
    if (!jit_diag_enabled() || !start || !end || end <= start)
        return;
    static unsigned long dumped_blocks = 0;
    if (dumped_blocks++ >= 16)
        return;
    const size_t byte_count = (size_t)(end - start);
    fprintf(stderr, "JIT_DIAG native_block guest_pc=%08x optlev=%d host=%p..%p bytes=%zu\n",
        (unsigned)guest_pc, level, (const void *)start, (const void *)end, byte_count);
    const size_t instruction_count = byte_count / sizeof(uae_u32);
    const size_t dump_count = instruction_count < 512 ? instruction_count : 512;
    const uae_u32 *instructions = (const uae_u32 *)start;
    for (size_t i = 0; i < dump_count; ++i) {
        const uae_u32 instruction = instructions[i];
        const uintptr address = (uintptr)&instructions[i];
        if ((instruction & 0xfc000000) == 0x14000000) {
            int64_t displacement = (int64_t)(instruction & 0x03ffffff);
            if (displacement & 0x02000000)
                displacement |= ~INT64_C(0x03ffffff);
            const uintptr destination = address + (uintptr)(displacement * 4);
            fprintf(stderr, "JIT_DIAG native_branch at=%p insn=%08x target=%p guest_pc=%08x\n",
                (const void *)address, instruction, (const void *)destination, (unsigned)guest_pc);
            jit_diag_check_target("native_block:B", address, destination,
                (uintptr)end, guest_pc);
        }
    }
    fflush(stderr);
}
...
extern "C" __attribute__((noinline)) void jit_diag_bad_target_breakpoint(
    const char *site, uintptr patch_address, uintptr target_address,
    uintptr written_end, uae_u32 guest_pc)
{
    fprintf(stderr,
        "JIT_DIAG BAD_TARGET site=%s patch=%p target=%p guest_pc=%08x "
        "cache=[%p,%p) written_end=%p instruction=%08x\n",
        site ? site : "unknown", (void *)patch_address, (void *)target_address,
        (unsigned)guest_pc, (void *)compiled_code,
        (void *)((uintptr)compiled_code + (uintptr)cache_size * 1024),
        (void *)written_end, patch_address ? *(const uae_u32 *)patch_address : 0);
    fflush(stderr);

    /* Diagnostics are explicitly enabled for this run. Stop before the invalid
       destination can be installed or executed, preserving the producer stack. */
    __builtin_trap();
}
    ...
    uintptr target_address, uintptr written_end, uae_u32 guest_pc)
{
    ...
    const uintptr cache_end = cache_start + (uintptr)cache_size * 1024;
    const uintptr popall_start = (uintptr)popallspace;
    const uintptr popall_end = popall_start + POPALLSPACE_SIZE;
    const bool patching_current_frontier = site &&
        strcmp(site, "write_jmp_target") == 0 && target_address == written_end;
    const bool in_unwritten_cache =
        target_address >= cache_start && target_address < cache_end &&
        target_address >= written_end && !patching_current_frontier;
    const bool requires_jit_target = site &&
        (strncmp(site, "get_handler_for_edge", 20) == 0 ||
         strcmp(site, "native_block:B") == 0);
    const bool in_popall = popallspace &&
        target_address >= popall_start && target_address < popall_end;
    const bool in_written_cache =
        target_address >= cache_start && target_address < written_end;
    const bool outside_jit_regions =
        requires_jit_target && !in_popall && !in_written_cache;

    if (in_unwritten_cache || outside_jit_regions) {
        jit_diag_bad_target_breakpoint(site, patch_address, target_address,
            written_end, guest_pc);
    }
}

/* Phase 1 hang diagnostic: log every write to cache_tags[cl].handler so we
   can see which one installs a bad value for the cacheline that the second
   pushall_call_handler dispatch is about to branch through. */
static void jit_diag_log_cache_write(uae_u32 cl, cpuop_func *handler, const char *site)
{
    if (!jit_diag_enabled())
        return;
    ...
    if (++count > 4000)
        return;
    fprintf(stderr, "JIT_DIAG cache_write cl=%u handler=%p site=%s\n", cl, (void*)handler, site);
    fflush(stderr);
}
...
static void disable_jit_runtime(const char* reason)
{
	...
	/* This CPU tree has no timer ownership hand-off API. The normal timer
	 * remains active; generated code reaches this path only after state has
	 * been materialized at the dispatcher boundary. */
}
...
extern "C" void jit_prepare_native_execute(void)
{
    ...
    if (jit_diag_enabled()) {
        ...
        if (calls < 200 || (calls % 5000) == 0) {
            fprintf(stderr, "JIT_DIAG prepare_native_execute call=%lu write_depth=%d\n",
                calls, jit_write_window_depth);
            fflush(stderr);
        }
        calls++;
    }
    /* pthread_jit_write_protect_np is per-thread.  Generated code must never
       inherit a writable/non-executable state from tracing or recompilation. */
    uae_vm_jit_write_protect(true);
#endif
}
...
static inline void remove_from_cl_list(blockinfo* bi)
{
    ...
    if (cache_tags[cl + 1].bi) {
        ...
        jit_diag_log_cache_write(cl, cache_tags[cl].handler, "remove_from_cl_list:next_bi");
    } else {
        cache_tags[cl].handler = (cpuop_func*)popall_execute_normal;
        jit_diag_log_cache_write(cl, cache_tags[cl].handler, "remove_from_cl_list:default");
    }
}
...
static inline void add_to_cl_list(blockinfo* bi)
{
    ...
    jit_diag_log_cache_write(cl, cache_tags[cl].handler, "add_to_cl_list");
}
...
static inline void block_need_recompile(blockinfo* bi)
{
    ...
    if (bi == cache_tags[cl + 1].bi) {
        cache_tags[cl].handler = (cpuop_func*)popall_execute_normal;
        jit_diag_log_cache_write(cl, cache_tags[cl].handler, "block_need_recompile");
    }
...
#if defined(CPU_AARCH64)
/* Keep the first ARM64 dispatch boundary on the C ABI.  The former generated
 * lookup reconstructed cacheline(regs.pc_p), loaded regs.cache_tags, and
 * branched through that value entirely in MAP_JIT code.  On macOS that path
 * intermittently transferred to an address beyond the translation cache even
 * though the matching C-side entry was valid.  A normal helper call preserves
 * LR and the callee-saved JIT base registers, and returns the same handler in
 * x0.  Set B2_JIT_DIRECT_TAG_LOOKUP=1 to retain the old emitter for A/B tests. */
static uintptr jit_lookup_dispatch_handler(void)
{
    ...
    if (jit_diag_enabled()) {
        ...
        if (calls < 200 || (calls % 5000) == 0) {
            fprintf(stderr,
                "JIT_DIAG dispatch_lookup call=%lu pc=%08x pc_p=%p cl=%u table=%p handler=%p bi=%p\n",
                calls, (unsigned)m68k_getpc(), (void *)regs.pc_p, cl,
                (void *)cache_tags, (void *)handler, (void *)cache_tags[cl + 1].bi);
            fflush(stderr);
        }
        calls++;
    }
...
{
    /* Generated and reference execution share the privilege, PC-advance and
       EXEC_RETURN termination contract in newcpu.cpp. */
...
static uintptr get_handler_for_edge(const blockinfo *source_bi, int edge_slot, uintptr addr)
{
    ...
    if (!forced_validated && bi->status == BI_ACTIVE &&
        ...
        const uintptr selected = h ? h : (uintptr)popall_execute_normal;
        uintptr written_end = (uintptr)get_target();
        if ((uintptr)current_compile_p > written_end)
            written_end = (uintptr)current_compile_p;
        jit_diag_check_target("get_handler_for_edge:direct", 0, selected,
            written_end, (uae_u32)addr);
        return selected;
    }
    ...
    if ((uintptr)current_compile_p > written_end)
        written_end = (uintptr)current_compile_p;
    jit_diag_check_target("get_handler_for_edge", 0, selected, written_end,
        (uae_u32)addr);
#endif
    return selected;
}
...
void writebyte(int address, int source)
{
    ...
    if (jit_force_all_special_mem() || (special_mem & S_WRITE) || distrust_byte() || jit_n_addr_unsafe)
...
void writeword(int address, int source)
{
    if (jit_strict_cache_disabled_coherence())
        jit_emitted_guest_memory_write = true;
    if (jit_force_all_special_mem() || (special_mem & S_WRITE) || distrust_word() || jit_n_addr_unsafe)
...
void writelong(int address, int source)
{
    if (jit_strict_cache_disabled_coherence())
        jit_emitted_guest_memory_write = true;
    if (jit_force_all_special_mem() || (special_mem & S_WRITE) || distrust_long() || jit_n_addr_unsafe)
...
void set_cache_state(int enabled)
{
    ...
    if (jit_diag_enabled()) {
        ...
        if (set_cache_state_calls <= 50 || (set_cache_state_calls % 5000) == 0) {
            fprintf(stderr, "JIT_DIAG set_cache_state called (enabled=%d cache_enabled=%d) total=%lu pc=0x%08x\n",
                enabled, cache_enabled, set_cache_state_calls, (unsigned)m68k_getpc());
            fflush(stderr);
        }
    }
    ...
    if (jit_strict_full_jit_env() && compiled_code && !requested_cache_state) {
        ...
        if (cache_enabled && !strict_cache_disable_boundary_seen) {
            jit_diag_last_flush_site = "set_cache_state:strict-boundary";
            flush_icache_hard(3);
        }
        cache_enabled = 1;
...
{
    if (compiled_code) {
        jit_diag_last_flush_site = "alloc_cache:reinit";
        flush_icache_hard(3);
#if defined(CPU_AARCH64)
...
STATIC_INLINE void create_popalls(void)
{
  ...
  /* We need to guarantee 16-byte stack alignment on x86 at any point
    ...
    {
        const char *disable_fpu_entry_sync = getenv("B2_JIT_DISABLE_ENTRY_FPU_SYNC");
        if (!(disable_fpu_entry_sync && *disable_fpu_entry_sync &&
              strcmp(disable_fpu_entry_sync, "0") != 0))
...
void build_comp(void)
{
    ...
    for (opcode = 0; opcode < 65536; opcode++) {
        ...
        if (!compfunctbl[cft_map(opcode)]) {
            ...
            /* Do not guess an alias by scanning for a merely similar
               instruction shape. Besides making startup quadratic, that can
               attach a generated handler whose embedded opcode fields do not
               match this encoding. A missing readcpu handler chain is safely
               left untranslated and handled by the interpreter. */
            if (f) {
                compfunctbl[cft_map(opcode)] = f;
                nfcompfunctbl[cft_map(opcode)] = nff;
...
void flush_icache_hard(int n)
{
    ...
    if (jit_diag_enabled()) {
        jit_diag_flush_icache_hard_calls++;
        fprintf(stderr, "JIT_DIAG flush_icache_hard called (n=%d), total=%lu, site=%s\n", n, jit_diag_flush_icache_hard_calls, jit_diag_last_flush_site);
        fflush(stderr);
        jit_diag_maybe_print();
    }
#endif
    ...
    while (bi) {
        ...
        jit_diag_log_cache_write(cacheline(bi->pc_p), cache_tags[cacheline(bi->pc_p)].handler, "flush_icache_hard:active");
    ...
    while (bi) {
        cache_tags[cacheline(bi->pc_p)].handler = (cpuop_func*)popall_execute_normal;
        jit_diag_log_cache_write(cacheline(bi->pc_p), cache_tags[cacheline(bi->pc_p)].handler, "flush_icache_hard:dormant");
        cache_tags[cacheline(bi->pc_p) + 1].bi = NULL;
        dbi = bi;
        bi = bi->next;
        free_blockinfo(dbi);
...
static inline void flush_icache_lazy(int v)
{
    ...
    while (bi) {
        ...
        if (bi->status == BI_INVALID || bi->status == BI_NEED_RECOMP) {
            if (bi == cache_tags[cl + 1].bi) {
                cache_tags[cl].handler = (cpuop_func*)popall_execute_normal;
                jit_diag_log_cache_write(cl, cache_tags[cl].handler, "flush_icache_lazy:invalid");
            }
            bi->handler_to_use = (cpuop_func*)popall_execute_normal;
            set_dhtu(bi, bi->direct_pen);
            bi->status = BI_INVALID;
            ...
            if (bi == cache_tags[cl + 1].bi) {
                ...
                jit_diag_log_cache_write(cl, cache_tags[cl].handler, "flush_icache_lazy:check");
            }
...
void compile_block(cpu_history* pc_hist, int blocklen, int totcycles)
{
    ...
    if (jit_strict_full_jit_env() &&
        ...
        const uae_u32 pc = (blocklen > 0 && pc_hist[0].location)
            ? jit_hostpc_to_macpc((uintptr)pc_hist[0].location) : 0xffffffffu;
    ...
    if (cache_enabled && compiled_code && currprefs.cpu_model >= 68020) {
        ...
        if (current_compile_p >= MAX_COMPILE_PTR) {
            jit_diag_last_flush_site = "compile_block:cache-full-pre";
...
endblock_done:
		...
		jit_diag_dump_native_block(block_m68k_pc, optlev,
			(const uae_u8 *)current_block_start_target, (const uae_u8 *)get_target());
        ...
        if (current_compile_p >= MAX_COMPILE_PTR) {
            jit_diag_last_flush_site = "compile_block:cache-full-post";
            flush_icache_hard(3);
        }
		jit_end_write_window();
    }
