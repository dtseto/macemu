...
static void *jit_watchdog_thread(void *)
{
	for (;;) {
		struct timespec interval = { 0, 250000000 };
		nanosleep(&interval, NULL);
		const uint64_t deadline = jit_watchdog_deadline_ns.load(std::memory_order_acquire);
		if (deadline && jit_watchdog_monotonic_ns() >= deadline) {
...
static void jit_watchdog_start_once(void)
{
	pthread_t thread;
	if (pthread_create(&thread, NULL, jit_watchdog_thread, NULL) == 0)
	...
	uae_u32 cl, uintptr handler, const void *bi, bool native_allowed)
{
	...
	if (remaining < 0) {
		const char *env = getenv("B2_JIT_DISPATCH_TRACE");
...
#endif
	/* Bootstrap each newly-created ARM64 dispatcher from an ordinary C frame.
	   This keeps the very first cache-miss/trace operation out of the generated
	   push/pop trampoline. Subsequent iterations use the normal native entry so
	   this is diagnostic scaffolding, not an interpreter-only execution mode. */
#if defined(CPU_AARCH64)
	static void *bootstrapped_dispatcher = nullptr;
#endif
	for (;;) {
#if defined(CPU_AARCH64)
			...
			write_log("JIT: ARM64: first execute_normal() returned pc=%08x pc_p=%p\n",
				...
				if (jit_diag_enabled()) {
					...
					if (dispatch_count <= 200 ||
						(dispatch_count & (dispatch_count - 1)) == 0) {
						fprintf(stderr, "JIT_DIAG pre_native_dispatch count=%lu pc=%08x pc_p=%p cl=%u handler=%p bi=%p\n",
							dispatch_count, (unsigned)m68k_getpc(),
							(void*)regs.pc_p, cl, (void*)handler, (void*)bi);
						fflush(stderr);
					}
				}
				...
				/* PocketShaver's dispatcher model is the important distinction here:
					...
					if (jit_diag_enabled()) {
						...
						safe_dispatch_count++;
						if (safe_dispatch_count <= 200 ||
							(safe_dispatch_count & (safe_dispatch_count - 1)) == 0) {
							fprintf(stderr,
		...
		if (use_sync_ticks && !use_retirement_ticks) {
			/* Drive the 60Hz tick by WALL-CLOCK at a safe block-dispatch
			   ...
			   Reading the host clock on every dispatcher return is needlessly
			   expensive: the required granularity is 16.67 ms, while normal JIT
			   dispatch runs many thousands of times per millisecond. Sample once
			   per 256 returns, matching the measured Previous AArch64 fix, but
			   keep the interrupt itself on this fully materialised boundary. */
		...
		/* The 2026 CPU core has no TimerPoll() hook. Its normal timer remains
		 * responsible for scheduling while this experimental dispatcher runs. */
		{
			extern int32 jit_countdown;
			if (jit_countdown < 0)
				jit_countdown = 10000000;
...
// Keep the ARM64 compatibility implementation in this translation unit.
...
#endif
#include "newcpu.h"
#if defined(CPU_aarch64) || defined(CPU_AARCH64) || defined(CPU_arm)
...
#if 0 // Debug trace helpers refer to JIT-private state after its compilation scope.
