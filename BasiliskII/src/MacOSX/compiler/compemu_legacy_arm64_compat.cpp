...
{
	/* The legacy generator uses x86 parity/no-parity for the M68K signed
	   conditions LT/GE: x86 PF held the synthesized N xor V predicate.
	   AArch64 has no parity condition, but its native LT/GE conditions test
	   the same architectural N/V relationship directly. */
	switch (cc) {
	...
	case 10: return NATIVE_CC_LT;
	case 11: return NATIVE_CC_GE;
...
void arm_AND_b(RW1 d, RR1 s) { and_b(d, s); }
void arm_AND_w(RW2 d, RR2 s) { and_w(d, s); }
void arm_AND_l(RW4 d, RR4 s) { and_l(d, s); }
void arm_EOR_b(RW1 d, RR1 s) { if (legacy_needflags_enabled()) jff_EOR_b(d, s); else jnf_EOR_b(d, s); }
void arm_EOR_w(RW2 d, RR2 s) { if (legacy_needflags_enabled()) jff_EOR_w(d, s); else jnf_EOR_w(d, s); }
void arm_EOR_l(RW4 d, RR4 s) { if (legacy_needflags_enabled()) jff_EOR_l(d, s); else jnf_EOR_l(d, s); }
...
void arm_ORR_b(RW1 d, RR1 s) { or_b(d, s); }
void arm_ORR_w(RW2 d, RR2 s) { or_w(d, s); }
void arm_ORR_l(RW4 d, RR4 s) { or_l(d, s); }
void legacy_ror_l_ri(RW4 d, uae_s32 i) { jnf_ROR_l_imm(d, i); }
...
void jit_install_fast_interpreter_overrides(void)
{
	...
	const char *disable_fast_zero = getenv("B2_JIT_DISABLE_FAST_ZERO");
	if (disable_fast_zero && *disable_fast_zero && strcmp(disable_fast_zero, "0") != 0) {
		if (jit_diag_enabled()) {
			fprintf(stderr, "JIT_DIAG preserving original opcode-0 handler=%p\n",
				(void *)jit_orig_op_0_0_ff);
			fflush(stderr);
		}
		installed = true;
		return;
...
static inline void jit_dispatch_interpreter_opcode(uae_u32 opcode, const char *path)
{
	...
	if (trace_remaining > 0) {
		trace_call = true;
		const uae_u8 *fetch = regs.pc_p;
		fprintf(stderr,
			"JITDISPATCH path=%s pc=%08x pc_p=%p bytes=%02x%02x "
			"opcode=%04x handler=%p spcflags=%08x\n",
			path,
			(unsigned)m68k_getpc(),
			(const void *)fetch,
			fetch ? (unsigned)fetch[0] : 0,
			fetch ? (unsigned)fetch[1] : 0,
	...
	if (handler == NULL)
		jit_abort("null interpreter handler: path=%s pc=%08x opcode=%04x pc_p=%p",
			path, m68k_getpc(), opcode & 0xffff, (void *)regs.pc_p);

	trace_call = trace_call && jit_diag_enabled();
	if (trace_call) {
		fprintf(stderr,
			"JIT_DIAG interp_before path=%s pc=%08x opcode=%04x handler=%p pc_p=%p\n",
			path, (unsigned)m68k_getpc(), (unsigned)(opcode & 0xffff),
			(void *)handler, (void *)regs.pc_p);
		fflush(stderr);
	}
	handler(opcode);
	if (trace_call) {
		fprintf(stderr,
			"JIT_DIAG interp_after path=%s pc=%08x opcode=%04x handler=%p pc_p=%p\n",
			path, (unsigned)m68k_getpc(), (unsigned)(opcode & 0xffff),
			(void *)handler, (void *)regs.pc_p);
		fflush(stderr);
	}
...
void exec_nostats(void)
{
	...
	for (;;) {
		...
		jit_dispatch_interpreter_opcode(opcode, "exec_nostats");
		if (trace_this) {
...
jit_pctrace_done:
		...
		for (;;) {
			...
			jit_dispatch_interpreter_opcode(opcode, "execute_normal");
			cpu_check_ticks();
...
extern "C" void jit_fpu_sync_to_shadow(void)
{
    ...
    if (jit_diag_enabled()) {
        fprintf(stderr, "JIT_DIAG fpu_sync_to_shadow enter sp=%p dirty=%08x\n",
            __builtin_frame_address(0), (unsigned)regs.jit_fp_dirty_mask);
        fflush(stderr);
    }
#endif
#if defined(CPU_aarch64) || defined(CPU_AARCH64)
...
#if defined(CPU_aarch64) || defined(CPU_AARCH64)
    if (jit_diag_enabled()) {
        fprintf(stderr, "JIT_DIAG fpu_sync_to_shadow exit sp=%p\n",
