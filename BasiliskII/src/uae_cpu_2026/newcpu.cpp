...
void Exception(int nr, uaecptr oldpc)
{
    ...
    if (((nr == 0xA || nr == 0xB) && currpc >= 0x04000500 && currpc <= 0x04000508) ||
        ...
        fprintf(stderr,
            "ALINE_EXC nr=%d currpc=%08x oldpc=%08x vector=%08x vbr=%08x atab1454=%08x atab1458=%08x sr=%04x intmask=%u spc=%08x d0=%08x d1=%08x d2=%08x d7=%08x a0=%08x a1=%08x a6=%08x a7=%08x ma6m4=%08x ma6=%08x ma6p4=%08x ma6p8=%08x ma6pc=%08x ma6p10=%08x ma6p14=%08x ma6p18=%08x ma6p1c=%08x sp0=%08x sp4=%08x sp8=%08x sp12=%08x sp16=%08x\n",
            nr,
            (unsigned)currpc,
            (unsigned)oldpc,
            ...
            (unsigned)m68k_dreg(regs,7),
            (unsigned)m68k_areg(regs,0),
            (unsigned)m68k_areg(regs,1),
            (unsigned)m68k_areg(regs,6),
            (unsigned)m68k_areg(regs,7),
            (unsigned)get_long(m68k_areg(regs,6) - 4),
            (unsigned)get_long(m68k_areg(regs,6) + 0),
            (unsigned)get_long(m68k_areg(regs,6) + 4),
            (unsigned)get_long(m68k_areg(regs,6) + 8),
            (unsigned)get_long(m68k_areg(regs,6) + 12),
            (unsigned)get_long(m68k_areg(regs,6) + 16),
            (unsigned)get_long(m68k_areg(regs,6) + 20),
            (unsigned)get_long(m68k_areg(regs,6) + 24),
            (unsigned)get_long(m68k_areg(regs,6) + 28),
...
void m68k_emulop_return(void)
{
	if (b2_test_retdump_enabled()) {
		...
		fprintf(stderr,
			...
			"A0=%08x A1=%08x A2=%08x A3=%08x A4=%08x A5=%08x A6=%08x A7=%08x SR=%04x NZCV=%08x X=%08x\n",
			(unsigned)m68k_dreg(regs, 0), (unsigned)m68k_dreg(regs, 1),
			(unsigned)m68k_dreg(regs, 2), (unsigned)m68k_dreg(regs, 3),
			(unsigned)m68k_dreg(regs, 4), (unsigned)m68k_dreg(regs, 5),
			(unsigned)m68k_dreg(regs, 6), (unsigned)m68k_dreg(regs, 7),
